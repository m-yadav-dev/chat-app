...
const ProfileContent = ({ authUser, updateProfile, isUpdatingProfile }) => {
  ...
  const currentAbout = authUser?.about || "";
  ...
  const onResetChanges = () => {
    ...
    setSelectedImage(currentProfilePic);
  ...
  const onSaveProfile = async () => {
    ...
    if (pendingProfilePic && pendingProfilePic !== currentProfilePic) {
  ...
  const trimmedName = fullName.trim();
  const fullNameChanged = trimmedName !== currentFullName;
  const aboutChanged = about !== currentAbout;
  const profilePicChanged =
    pendingProfilePic !== null && pendingProfilePic !== currentProfilePic;

  const hasChanges =
    fullNameChanged || aboutChanged || profilePicChanged;
...
const ProfilePage = () => {
  ...
  const profileKey = `${authUser?._id ?? "no-user"}:${authUser?.updatedAt ?? ""}:${authUser?.profilePic ?? ""}:${authUser?.fullName ?? ""}:${authUser?.about ?? ""}`;

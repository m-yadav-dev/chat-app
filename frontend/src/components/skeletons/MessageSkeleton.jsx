...
const MessageSkeleton = () => {
  // 6 dummy messages ka ek array banayenge loading effect ke liye
  const skeletonMessages = Array.from({ length: 6 });

  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-slate-950 w-full h-full">
      {skeletonMessages.map((_, idx) => {
        // Even index wale messages ko left (Receiver) aur Odd ko right (Sender) dikhayenge
        const isMe = idx % 2 !== 0;
        ...
        return (
          <div key={idx} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
            <div className={`flex gap-3 max-w-[85%] sm:max-w-[70%] ${isMe ? "flex-row-reverse" : "flex-row"}`}>
              ...
              {/* Avatar Skeleton (Sirf receiver side dikhane ke liye standard approach) */}
              {!isMe && (
              ...
              <div className={`flex flex-col gap-2 ${isMe ? "items-end" : "items-start"}`}>
                ...
                <div 
                  ...
                  // Random widths taaki messages natural lagein (sab ek barabar na hon)
